...
admissions_tble %>%
  ...
  theme_bw() +
  geom_bar() + 
  labs(title = "Figure 4. Numbers of admissions for each hour",
       x = "Hour") 
```
...
patients_tble %>%
  ...
  geom_bar() +
  labs(title = "Figure 5. Patient number by gender")
```
...
patients_tble %>%
  ...
  labs(title = "Figure 6. Density plot of anchor age")
```
...
patients_tble %>%
  ...
  labs(title = "Figure 6. Density plot of anchor age by gender")
```
...
<p style="color:DarkRed;">**Answer:**There are 8398756 rows in the compressed `labevents.csv.gz`. Note: this number is the count of rows in the compressed data instead of original data, so it does not stand for the number of records. </p>
