...
icu_cohort <- icustays_tble %>% 
  ...
  mutate(
    thirty_day_mort = case_when(is.na(dod) ~ 0,
                                as.Date(admittime) + as.integer(30) > dod ~ 1,
                                as.Date(admittime) + as.integer(30) < dod ~ 0)
  ) %>%
  # print results
  print(width = Inf)
```
...
\textcolor{red}{For this part, please refer to the Shiny app in the `mimiciv_shiny` folder.}